...
def render_atoms(atom_dict, base_x, base_y, color):
    ...
    for pos in atom_dict.values():
        ...
        screen_x = base_x + (x_fisico * CELL_SIZE) + (CELL_SIZE / 2)
        screen_y = base_y + (y_fisico * CELL_SIZE) + (CELL_SIZE / 2)
...
BASE_X_AMBIENTE = -400
BASE_Y_AMBIENTE = -100
BASE_X_TANQUE = BASE_X_AMBIENTE + (A_LARG * CELL_SIZE) + GRID_SPACING
BASE_Y_TANQUE = -100
...
# MUDANÇA: Não fazemos nada aqui. O tanque começa cheio.
...
for frame in range(movs_restantes):
    ...
    atom.clear()
    hud.clear()
    render_atoms(atomos_ambiente, BASE_X_AMBIENTE, BASE_Y_AMBIENTE, "royalblue")
    render_atoms(atomos_tanque, BASE_X_TANQUE, BASE_Y_TANQUE, "darkorange")
    
    # MUDANÇA: O 'frame' agora começa do 0 real (não +2) pois não teve movimento prévio
    hud.write(f"Movimento: {frame + 1}\nCarro na posição: {posicao_carro}", 
